      ...
      return [r].flat().reduce((s4, a2) => {
        if (a2 != null) {
          ...
          if (u === "string" || u === "number") s4.push(a2);
            ...
            let c = Array.isArray(a2) ? o(n, a2) : Object.entries(a2).map(([f, g4]) => n === "style" && (g4 || g4 === 0) ? `${f.replace(/([a-z])([A-Z])/g, "$1-$2").toLowerCase()}:${g4}` : g4 ? f : void 0);
            s4 = c.length ? s4.concat(c.filter((f) => !!f)) : s4;
          }
        }
        return s4;
      }, i2);
  ...
  if (p2(e)) {
    ...
    if (z2(a2, k2)) {
      let n = a2.replaceAll(k2, (l2) => {
        let c = l2.replace(/{|}/g, "").split(".").filter((m2) => !s4.some((d2) => z2(m2, d2)));
...
function le(e, t2) {
  ...
  function r(n, l2) {
    let o = [], c = 0, m2 = "", d2 = null, u = 0;
    for (; c <= n.length; ) {
      let h2 = n[c];
      if ((h2 === '"' || h2 === "'" || h2 === "`") && n[c - 1] !== "\\" && (d2 = d2 === h2 ? null : h2), !d2 && (h2 === "(" && u++, h2 === ")" && u--, (h2 === "," || c === n.length) && u === 0)) {
        let f = m2.trim();
        f.startsWith("dt(") ? o.push(le(f, l2)) : o.push(s4(f)), m2 = "", c++;
        continue;
      }
      h2 !== void 0 && (m2 += h2), c++;
    }
  ...
  for (let n = i2.length - 1; n >= 0; n--) {
    let [l2, o] = i2[n], c = e.slice(l2 + 3, o), m2 = r(c, t2), d2 = t2(...m2);
...
var ue2 = (e = {}, t2, r, s4) => {
  if (t2) {
    let { variable: i2, options: a2 } = S.defaults || {}, { prefix: n, transform: l2 } = (e == null ? void 0 : e.options) || a2 || {}, o = z2(t2, k2) ? t2 : `{${t2}}`;
...
function de(e, t2 = {}) {
  ...
  for (; o.length; ) {
    ...
    for (let u in m2) {
      let h2 = m2[u], f = ve(h2), p3 = z2(u, a2) ? Q2(d2) : Q2(d2, ee(u));
      if (i(f)) o.push({ node: f, path: p3 });
      else {
        let y2 = ae(p3), R2 = Y(f, p3, s4, [a2]);
        Re(l2, y2, R2);
        let T2 = p3;
...
var b2 = { regex: { rules: { class: { pattern: /^\.([a-zA-Z][\w-]*)$/, resolve(e) {
  ...
  var R2, T2, j, O2, M2, z3, V;
  let { preset: a2, options: n } = t2, l2, o, c, m2, d2, u, h2;
  if (s2(a2) && n.transform !== "strict") {
    let { primitive: L, semantic: te, extend: re } = a2, f = te || {}, { colorScheme: K2 } = f, A2 = v3(f, ["colorScheme"]), x2 = re || {}, { colorScheme: X } = x2, G2 = v3(x2, ["colorScheme"]), p3 = K2 || {}, { dark: U3 } = p3, B = v3(p3, ["dark"]), y2 = X || {}, { dark: I } = y2, H = v3(y2, ["dark"]), W2 = s2(L) ? this._toVariables({ primitive: L }, n) : {}, q2 = s2(A2) ? this._toVariables({ semantic: A2 }, n) : {}, Z = s2(B) ? this._toVariables({ light: B }, n) : {}, pe = s2(U3) ? this._toVariables({ dark: U3 }, n) : {}, fe2 = s2(G2) ? this._toVariables({ semantic: G2 }, n) : {}, ye = s2(H) ? this._toVariables({ light: H }, n) : {}, Se = s2(I) ? this._toVariables({ dark: I }, n) : {}, [Me, ze] = [(R2 = W2.declarations) != null ? R2 : "", W2.tokens], [Ke, Xe] = [(T2 = q2.declarations) != null ? T2 : "", q2.tokens || []], [Ge, Ue] = [(j = Z.declarations) != null ? j : "", Z.tokens || []], [Be, Ie] = [(O2 = pe.declarations) != null ? O2 : "", pe.tokens || []], [He, We] = [(M2 = fe2.declarations) != null ? M2 : "", fe2.tokens || []], [qe, Ze] = [(z3 = ye.declarations) != null ? z3 : "", ye.tokens || []], [Fe, Je] = [(V = Se.declarations) != null ? V : "", Se.tokens || []];
  ...
  return { primitive: { css: l2, tokens: o }, semantic: { css: c, tokens: m2 }, global: { css: d2, tokens: u }, style: h2 };
}, getPreset({ name: e = "", preset: t2 = {}, options: r, params: s4, set: i2, defaults: a2, selector: n }) {
  var f, x2, p3;
  let l2, o, c;
  if (s2(t2) && r.transform !== "strict") {
    let y2 = e.replace("-directive", ""), m2 = t2, { colorScheme: R2, extend: T2, css: j } = m2, O2 = v3(m2, ["colorScheme", "extend", "css"]), d2 = T2 || {}, { colorScheme: M2 } = d2, z3 = v3(d2, ["colorScheme"]), u = R2 || {}, { dark: V } = u, L = v3(u, ["dark"]), h2 = M2 || {}, { dark: te } = h2, re = v3(h2, ["dark"]), K2 = s2(O2) ? this._toVariables({ [y2]: g3(g3({}, O2), z3) }, r) : {}, A2 = s2(L) ? this._toVariables({ [y2]: g3(g3({}, L), re) }, r) : {}, X = s2(V) ? this._toVariables({ [y2]: g3(g3({}, V), te) }, r) : {}, [G2, U3] = [(f = K2.declarations) != null ? f : "", K2.tokens || []], [B, I] = [(x2 = A2.declarations) != null ? x2 : "", A2.tokens || []], [H, W2] = [(p3 = X.declarations) != null ? p3 : "", X.tokens || []], q2 = this.transformCSS(y2, `${G2}${B}`, "light", "variable", r, i2, a2, n), Z = this.transformCSS(y2, H, "dark", "variable", r, i2, a2, n);
    l2 = `${q2}${Z}`, o = [.../* @__PURE__ */ new Set([...U3, ...I, ...W2])], c = m(j, { dt: E2 });
  }
  ...
  var c, m2;
  let a2 = e.replace("-directive", ""), { preset: n, options: l2 } = t2, o = ((c = n == null ? void 0 : n.components) == null ? void 0 : c[a2]) || ((m2 = n == null ? void 0 : n.directives) == null ? void 0 : m2[a2]);
  ...
  let n = this.getCommon({ name: e, theme: t2, params: r, set: i2, defaults: a2 }), l2 = Object.entries(s4).reduce((o, [c, m2]) => o.push(`${c}="${m2}"`) && o, []).join(" ");
  return Object.entries(n || {}).reduce((o, [c, m2]) => {
    if (i(m2) && Object.hasOwn(m2, "css")) {
      let d2 = G(m2.css), u = `${c}-variables`;
  ...
  var c;
  let n = { name: e, theme: t2, params: r, set: i2, defaults: a2 }, l2 = (c = e.includes("-directive") ? this.getPresetD(n) : this.getPresetC(n)) == null ? void 0 : c.css, o = Object.entries(s4).reduce((m2, [d2, u]) => m2.push(`${d2}="${u}"`) && m2, []).join(" ");
  ...
  let a2 = function(l2, o = {}, c = []) {
    ...
    Object.entries(l2).forEach(([m2, d2]) => {
      let u = z2(m2, t2.variable.excludedKeyRegex) ? o : o ? `${o}.${oe2(m2)}` : oe2(m2), h2 = c ? `${c}.${m2}` : m2;
      i(d2) ? n(d2, u, h2) : (i2[u] || (i2[u] = { paths: [], computed: (f, x2 = {}, p3 = []) => {
  ...
  return n.length === 1 ? n[0].value : n.reduce((o = {}, c) => {
    let u = c, { colorScheme: m2 } = u, d2 = v3(u, ["colorScheme"]);
    return o[m2] = d2, o;
  }, void 0);
  ...
  if (s2(t2)) {
    ...
    if (s4 !== "style") {
      ...
      t2 = r === "dark" ? c.reduce((m2, { type: d2, selector: u }) => (s2(u) && (m2 += u.includes("[CSS]") ? u.replace("[CSS]", t2) : this.getSelectorRule(u, l2, d2, t2)), m2), "") : C2(l2 != null ? l2 : ":root", t2);
    }
    if (o) {
      let c = { name: "primeui", order: "primeui" };
      i(o) && (c.name = m(o.name, { name: e, type: s4 })), s2(c.name) && (t2 = C2(`@layer ${c.name}`, t2), a2 == null || a2.layerNames(c.name));
    }
    return t2;
  }
  return "";
...
//# sourceMappingURL=chunk-WMHBMRMU.js.map
